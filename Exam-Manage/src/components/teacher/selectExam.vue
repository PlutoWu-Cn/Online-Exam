...
<template>
  <div class="exam">
    <el-row :gutter="20">
      <el-col :span="8">
        <el-input placeholder="请输入试卷名称" class="input" v-model="name">
          <el-button
            slot="append"
            icon="el-icon-search"
            @click="search"
    ...
    <el-table :data="pagination.records" border v-model="form">
      <el-table-column
        fixed="left"
        prop="source"
        label="试卷名称"
    ...
    search() {
      ...
      this.$axios("/api/exams").then(res => {
        if (res.data.code == 200) {
          ...
          this.allexam.map(function(item) {
            if (item.source.search(examname) != -1) {
              newData1.push(item);
            }
          });
        }
